    ...
    <table>
        ...
        <tbody>
            @foreach($transactions as $transaction)
                ...
                <tr>
                    <td colspan="6">
                        <div class="section-heading" style="margin-bottom: 3px">Detail Produk dan Layanan</div>
                        <table>
                            <thead>
                                <tr>
                                    <th>Nama Produk</th>
                            ...
                            <tbody>
                                @foreach($transaction->details as $detail)
                                    <tr class="product-row">
                                        ...
                                        <td>{{ $detail->service->name_service ?? '-' }}</td>
                                        <td>{{ $detail->quantity }}</td>
                                        <td>{{ number_format($detail->harga_product, 0, ',', '.') }}</td>
                                        <td>{{ number_format($detail->harga_service, 0, ',', '.') }}</td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
